...
"""
NOTE: These mentioned apis are used for all type of saas sites to allow login to NexTash Cloud
...
def send_verification_code(domain: str, route: str = ""):
	...
	if not team_info or not team_info.get("enabled"):
		frappe.throw("Your NexTash Cloud team is disabled currently.")
...
def check_if_user_can_login(team_info, site_info):
	if team_info.get("enforce_2fa"):
		frappe.throw(
			"Sorry, you cannot login with this method as 2FA is enabled. Please visit https://nextash.cloud/dashboard to login."
		)
	if (
	...
	if not (site_info.get("standby_for") or site_info.get("standby_for_product")):
		frappe.throw("Only SaaS sites are allowed to login to NexTash Cloud via current method.")


def send_email_with_verification_code(email, otp):
	if frappe.conf.developer_mode:
		print("\nVerification Code for login to NexTash Cloud:")
		print(f"\nOTP for {email}:")
		print(otp)
		print()
	else:
		frappe.sendmail(
			recipients=email,
			subject="Verification Code for NexTash Cloud Login",
