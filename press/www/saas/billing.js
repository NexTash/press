const frappe_cloud_base_endpoint = 'https://nextash.cloud';
...
// NexTash Cloud login related
function initiateRequestForLoginToFrappeCloud() {
	frappe.confirm(
		'Are you sure you want to login to NexTash Cloud dashboard ?',
		() => {
			requestLoginToFC();
		},
	);
...
function requestLoginToFC(freezing_msg) {
	frappe.request.call({
		...
		freeze: true,
		freeze_message: freezing_msg || 'Initiating login to NexTash Cloud',
		success: function (r) {
			showFCLogindialog(r.message.email);
			setErrorMessage('');
		},
		error: function (r) {
			frappe.throw('Failed to login to NexTash Cloud. Please try again');
		},
	});
}
...
function showFCLogindialog(email) {
	if (!window.fc_login_dialog) {
		var d = new frappe.ui.Dialog({
			title: __('Login to NexTash Cloud'),
		...
		frappe.request.call({
			...
			success: function (r) {
				if (r.login_token) {
					...
					frappe.msgprint({
						title: __('NexTash Cloud Login Successful'),
						indicator: 'green',
						message: __(
							`<p>You will be redirected to NexTash Cloud soon.</p><p>If you haven\'t been redirected, <a href="${frappe_cloud_base_endpoint}/api/method/press.api.developer.saas.login_to_fc?token=${r.login_token}" target="_blank">Click here to login</a></p>`,
						),
					});
				} else {
