...
function get_subscription_url() {
	if (
		...
		return `https://nextash.cloud/dashboard/app-trial/setup/${frappe.boot.subscription_conf.app_trial}#subscription`;
	}
	let sitename = frappe.boot.sitename;
	return `https://nextash.cloud/dashboard/sites/${sitename}`;
}
